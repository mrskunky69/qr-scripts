...
local PainkillersDict = "mech_inventory@eating@multi_bite@wedge_a4-2_b0-75_w8_h9-4_eat_cheese"
local PainkillersAnim = "quick_right_hand"
...
RegisterNetEvent('hospital:client:UsePainkillers', function(itemName)
    ...
    QRCore.Functions.Progressbar("use_bandage", Lang:t('progress.painkillers'), 3000, false, true, {
		...
		TriggerServerEvent("hospital:server:TreatWounds", false)
        onPainKillers = true
        if painkillerAmount < 1 then
            painkillerAmount = painkillerAmount + 3
        end
    end, function() -- Cancel
        StopAnimTask(ped, PainkillersDict, PainkillersAnim, 1.0)
        QRCore.Functions.Notify(Lang:t('error.canceled'), 'error')
